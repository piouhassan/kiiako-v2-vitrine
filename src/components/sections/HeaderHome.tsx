...
import {
  ...
  HelpCircle, Newspaper, Truck, Building2, ChevronDown, ArrowRight, MapPin
} from 'lucide-react';
          ...
          <div className="max-w-[1400px] w-full mx-auto flex items-center justify-between">
            <div className='p-2'>
              <Link href="/" className="flex items-center">
                ...
                <figure className="lg:max-w-[170px] lg:block hidden">
                  <img src={isScrolled ? "/logo.png" : "/logo_blanc.png"} alt="Kiiako" className="dark:hidden block" />
                  <img src="/logo_blanc.png" alt="Kiiako" className="dark:block hidden" />
                </figure>
                <figure className="max-w-[44px] sm:max-w-[170px] max-w-[120px] lg:hidden block">
                  <img src={isScrolled ? "/logo.png" : "/logo_blanc.png"} alt="Kiiako" className="w-full dark:hidden block" />
            ...
            <nav className="hidden xl:flex items-center overflow-visible">
              <ul className="flex  overflow-visible">
                {/* Solutions Menu */}
                <li
                    className={`relative nav-item cursor-pointer py-2.5 overflow-visible ${openMenu === 'solutions' ? 'active' : ''}`}
                    onMouseEnter={() => {
                      cancelHideMenu();
                ...
                >
                  <a href="#" className={`flex items-center gap-1 px-4 py-2 border-b-2 rounded-none text-tagline-1 font-normal transition-all duration-200 ${
                      openMenu === 'solutions'
                          ? 'border-primary-500  text-primary-500 dark:text-primary-400'
                          : 'border-transparent hover:border-primary-500 ' + (isScrolled ? 'text-secondary hover:text-secondary dark:text-accent dark:hover:text-accent' : 'text-white hover:text-white')
                  }`}>
                                ...
                                >
                                  ...
                                  <div className="relative z-10 flex-1">
                                    <div className="flex items-center justify-between gap-2">
                                      <p className="text-tagline-1 font-normal text-secondary dark:text-accent">{t(feature.title)}</p>
                                      <ArrowRight className="w-4 h-4 opacity-0 group-hover:opacity-100 -translate-x-2 group-hover:translate-x-0 transition-all duration-300 text-primary-500" />
                                    </div>
                          ...
                          {links.map((link) => (
                              <li key={link.id}>
                                ...
                                >
                                  ...
                                  <ArrowRight className="w-4 h-4 opacity-0 group-hover:opacity-100 -translate-x-2 group-hover:translate-x-0 transition-all duration-300 text-secondary dark:text-accent relative z-10" />
                                </Link>
                              </li>
                          ))}
                        </ul>
                              ...
                              <ul className="space-y-1">
                                {section.items.map((item, itemIndex) => (
                                    <li key={itemIndex}>
                                      ...
                                      >
                                        <div className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 w-full h-full bg-background-3 dark:bg-background-7 opacity-0 group-hover:opacity-100 rounded-[10px] z-0 transition-all duration-400"></div>
                                        <p className="text-tagline-1 font-normal text-secondary dark:text-accent relative z-10">{t(item.label)}</p>
                  ...
                  >
                    <div className="flex gap-5">
                      ...
                      <div className="flex-1 space-y-1">
                        <p className="text-tagline-2 font-medium text-secondary/50 dark:text-accent/50 px-1 mb-2">Livraisons</p>
                        {[
                          { href: '/solutions/livreur-inscription', icon: Truck, labelKey: 'header.menu.services.1', desc: 'Inscrivez-vous, choisissez vos zones, livrez et gagnez.' },
                          { href: '/solutions/entreprise-livraison', icon: Building2, labelKey: 'header.menu.services.2', desc: 'Enregistrez votre compagnie et gérez votre flotte.' },
                          { href: '/solutions/livraisons', icon: MapPin, label: 'Comment ça marche ?', desc: 'Zones, missions, tarifs, paiements.' }
                        ].map((item) => {
                          ...
                          return (
                            <Link key={item.href} href={item.href} className={`p-3 rounded-2xl flex items-start gap-3 transition-all duration-300 relative group ${active ? 'is-active' : ''}`}>
                              ...
                              <div className="relative z-10 flex-1">
                                ...
                                <p className="text-[14px] text-secondary/50 dark:text-accent/50 leading-snug">{item.desc}</p>
                              </div>
                      ...
                      <div className="w-[240px] shrink-0">
                        <p className="text-tagline-2 font-medium text-secondary/50 dark:text-accent/50 px-1 mb-2">Essayez le Bot</p>
                        ...
                        <Link href="/solutions/bot-whatsapp" className="block bg-[#EFEAE2] dark:bg-[#0B141A] rounded-[20px] p-3.5 border border-stroke-1 dark:border-white/5 relative overflow-hidden group hover:-translate-y-1 hover:shadow-lg transition-all duration-300">
                          ...
                          <div className="flex items-center gap-2.5 relative z-10 mb-3">
                            ...
                            <div className="flex-1 min-w-0">
                              ...
                              <p className="text-[10.5px] text-[#00A884] font-medium">En ligne</p>
                            </div>
                          </div>
                          ...
                          <div className="relative z-10 space-y-2 mt-1">
                            ...
                            <div className="bg-white dark:bg-[#202C33] p-2.5 rounded-2xl rounded-tl-sm shadow-[0_1px_2px_rgba(0,0,0,0.05)] text-[12px] text-[#111B21] dark:text-[#E9EDEF] border border-black/5 dark:border-white/5 leading-snug w-[85%] relative before:absolute before:-left-2 before:top-0 before:w-0 before:h-0 before:border-[6px] before:border-transparent before:border-t-white dark:before:border-t-[#202C33] before:border-r-white dark:before:border-r-[#202C33]">
                              Gérez vos missions et boutique ici 👇
                            </div>
                            ...
                            <div className="bg-[#D9FDD3] dark:bg-[#005C4B] p-2.5 rounded-2xl rounded-tr-sm shadow-[0_1px_2px_rgba(0,0,0,0.05)] text-[11.5px] text-[#111B21] dark:text-[#E9EDEF] border border-black/5 dark:border-white/5 ml-auto w-[85%] relative before:absolute before:-right-2 before:top-0 before:w-0 before:h-0 before:border-[6px] before:border-transparent before:border-t-[#D9FDD3] dark:before:border-t-[#005C4B] before:border-l-[#D9FDD3] dark:before:border-l-[#005C4B]">
                              <p className="font-semibold text-[10.5px] text-[#00A884] dark:text-[#53bdeb] mb-1">Commandes rapides :</p>
                ...
                <li className="relative nav-item cursor-pointer py-2.5">
                  <Link href="/tarifs" className={`flex items-center gap-1 px-4 py-2 border-b-2 rounded-none text-tagline-1 font-normal transition-all duration-200 ${
                      pathname === '/tarifs'
                          ? 'border-primary-500  text-primary-500 dark:text-primary-400'
                          : 'border-transparent hover:border-primary-500 ' + (isScrolled ? 'text-secondary hover:text-secondary dark:text-accent dark:hover:text-accent' : 'text-white hover:text-white')
                  }`}>
      ...
      <aside className={`sidebar fixed top-0 right-0 w-full sm:w-[400px] transition-all duration-300 h-screen bg-white dark:bg-background-8 sm:rounded-l-3xl xl:hidden z-[9999] flex flex-col ${isOpen ? 'translate-x-0 shadow-2xl' : 'translate-x-full'}`}>
              ...
              <div className={`overflow-hidden transition-all duration-300 ${openMobileSubMenu === 'services' ? 'max-h-[1000px] opacity-100 mt-2' : 'max-h-0 opacity-0'}`}>
                <ul className="pl-4 space-y-1 border-l-2 border-stroke-1 dark:border-white/5 ml-3 pb-2 pt-1">
                  {[
                    ...
                    { href: '/solutions/livraisons', icon: MapPin, labelKey: '', label: 'Comment ça marche ?' }
                  ].map((item, idx) => {
                    const Icon = item.icon;
                    const lbl = item.label || t(item.labelKey);
                    return (
                      <li key={idx}>
                  ...
                  <li>
                    <Link href="/solutions/bot-whatsapp" onClick={close} className="flex items-center gap-3 p-3 rounded-xl hover:bg-background-3 dark:hover:bg-background-7 transition-colors text-secondary dark:text-accent mt-2 border border-stroke-1 dark:border-white/5 bg-[#EFEAE2] dark:bg-[#0B141A]">
                      ...
                      <div>
                        <p className="font-bold text-[13px] text-[#111B21] dark:text-white leading-tight">Missions WhatsApp</p>
                        <p className="text-[11px] text-[#00A884] font-medium">Voir le simulateur</p>
                      </div>
                    </Link>
                  </li>
                </ul>
        ...
        <div className="p-5 sm:p-8 shrink-0 border-t border-stroke-1 dark:border-white/10 bg-background-3 dark:bg-background-8">
          <Link 
            ...
            onClick={close}
            className="btn btn-md w-full shadow-lg btn-primary dark:btn-accent hover:btn-primary"
          >
            <span>{t('header.nav.start')}</span>
          </Link>
        </div>
